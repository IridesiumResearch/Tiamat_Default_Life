...
#      where it lives: MIT under stubs/ and in AGENTS.md (both vendored from
#      the engine's api/), GPL-3.0-only everywhere else. This rule
...
# required to.
REQUIRED_EXTENSIONS='\.(rs|lua|py|sh)$'
...
if [ "${#wrong_license[@]}" -gt 0 ]; then
    ...
    printf '  %s\n' "${wrong_license[@]}"
    echo
    echo "  Files under stubs/ and AGENTS.md are the engine's api/, MIT so mod authors can vendor them into"
...
if [ "$status" -ne 0 ]; then
  ...
  (`--` for Lua, `#` for shell and Python, `<!-- ... -->` for Markdown; MIT under stubs/.)
EOF
else
