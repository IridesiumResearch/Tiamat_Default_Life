...
pigs, horses, stags, goats, bears, crows and bats.
...
mods/tiamat_default_life/   the mod (this is what the engine loads)
  ...
  sounds/*.ogg              recordings from the sound library: the creatures' voices and the sea
                            over your head, brought in by tools/import_sounds.py
  sounds/*.wav              generated placeholders from tools/make_sounds.py, until the library has takes
tests/native/               the mod run through the engine's real script VM, and the icon hasher
tools/                      the asset generators (stdlib Python only; import_sounds.py needs ffmpeg)
...
`tools/import_sounds.py` names each take and converts it to what the
engine prefers: mono Ogg Vorbis (a voice is placed in the world and panned
by the engine), 44.1 kHz, at one loudness. To use another take, point its
line at it and run `python tools/import_sounds.py --force`. The rest (eating,
drinking, hurting, dying, bubbles, fire, the explosion, the bed) are still
the generated placeholders from `tools/make_sounds.py`.
...
Nine so far, in `creatures.lua` as data over the system in `mobs.lua`:
...
it is long; the goat at `--length 4.0 --rename eating=sneak`):
